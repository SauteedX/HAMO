...
bool AudioPlayer::setupSPI() {
    ...
    for (int i = 0; i < 7; i++) csPins[i] = duePins[i];
#elif defined(ARDUINO_ARC32_TOOLS)
    int pins101[] = {10, 4, 7, 8, 9, 5, 6};
    for (int i = 0; i < 7; i++) csPins[i] = pins101[i];
#else
    int generalPins[] = {10, 4, 7, 8, 9, 5, 6};
    for (int i = 0; i < 7; i++) csPins[i] = generalPins[i];
#endif
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (SD.begin(csPins[i])) {
            ...
            // Genuino 101: 커서 꼬임 방지 더미 open/close (워크어라운드)
            { File dummy = SD.open("/"); dummy.close(); }
...
bool AudioPlayer::reinitializeSD() {
    ...
    SPI.end(); delay(100); SPI.begin();
#if defined(ARDUINO_ARC32_TOOLS)
    SPI.setClockDivider(128);
#else
    SPI.setClockDivider(SPI_CLOCK_DIV128);
#endif
    pinMode(workingCSPin, OUTPUT); digitalWrite(workingCSPin, HIGH); delay(500);
    Serial.print("CS 핀 "); Serial.print(workingCSPin); Serial.print("으로 재초기화 시도... ");
    if (SD.begin(workingCSPin)) {
        Serial.println("성공!");
        File root = SD.open("/");
        if (root) { Serial.println("✓ 루트 디렉토리 접근 성공"); root.close(); return true; }
        else { Serial.println("✗ 루트 디렉토리 접근 실패"); return false; }
    } else {
        Serial.println("실패!"); return false;
    }
}
...
void AudioPlayer::setVolume(uint8_t volume) {
    ...
    Serial.print("볼륨 설정: "); Serial.println(volume);
}

void AudioPlayer::listFiles() {
    Serial.println("\n=== 파일 목록 (Genuino 101 호환) ===");
    if (workingCSPin == -1) {
        Serial.println("SD 카드가 초기화되지 않았습니다!"); return;
    }
    Serial.print("CS 핀 "); Serial.print(workingCSPin); Serial.println(" 사용 중");

    // Genuino 101 커서 버그 방지: 더미 open/close
    { File dummy = SD.open("/"); dummy.close(); }
    ...
    if (!root) {
        ...
        if (!reinitializeSD()) {
            Serial.println("SD 카드 재초기화 실패!"); return;
        }
        root = SD.open("/");
        if (!root) {
            Serial.println("재초기화 후에도 루트 디렉토리 열기 실패!"); return;
        }
    }

    Serial.println("루트 디렉토리 열기 성공");
    Serial.println("파일 목록:");
    int totalFiles = 0, wavFiles = 0;
    ...
    while (true) {
        ...
        Serial.print(totalFiles); Serial.print(". "); Serial.print(fileName);
        Serial.print(" ("); Serial.print(entry.size()); Serial.print(" bytes");
        if (entry.isDirectory()) Serial.print(", DIR");
        else {
            Serial.print(", FILE");
            String upperName = fileName; upperName.toUpperCase();
            if (upperName.endsWith(".WAV")) {
                Serial.print(" <-- WAV 파일!"); wavFiles++;
            }
        }
        Serial.println(")");
        entry.close();
    ...
    root.close();
    Serial.print("총 "); Serial.print(totalFiles); Serial.print("개 항목, ");
    Serial.print(wavFiles); Serial.println("개 WAV 파일");

    if (totalFiles == 0) {
        Serial.println("*** 파일이 감지되지 않음 - 하드웨어 문제일 가능성 ***");
        Serial.println("해결 방법: SD카드 재삽입/전원 리셋/다른 카드 실험");
    } else if (wavFiles == 0) {
...
bool AudioPlayer::selectAndPlayFile() {
    ...
    while (!Serial.available() && (millis() - startTime < 10000)) delay(100);

    if (!Serial.available()) {
        Serial.println("시간 초과"); return false;
    }

    int selection = Serial.parseInt();
    Serial.print("선택: "); Serial.println(selection);

    if (selection <= 0) {
        Serial.println("잘못된 번호"); return false;
    }
    ...
    File root = SD.open("/");
    if (!root) {
        Serial.println("루트 디렉토리 열기 실패"); return false;
    }
    int currentIndex = 0;
    String selectedFile;
    while (true) {
        ...
        if (!entry.isDirectory()) {
            ...
            if (currentIndex == selection) {
                selectedFile = String(entry.name()); entry.close(); break;
            }
        }
        entry.close();
    }
    ...
    if (selectedFile.length() > 0) {
        Serial.print("재생: "); Serial.println(selectedFile);
...
bool AudioPlayer::selectAndPlayFile(int fileNum) {
    // fileNum에 따라 파일 재생(구현)
}
